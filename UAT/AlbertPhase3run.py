...
import modulepagetree_phase4 as mpt
...
import modulenewtab_phase4 as mn
import moduleemptypage as mepg
import modulebrokenlinks_phase4 as mbl
import moduletranslation_phase4 as mtr
import moduletvar_phase4 as mtvar
# import moduletransnemptypage as mte
# import module_trans_gram_empt as mtge
import moduletransnempty_phase4 as mtea
...
def module(acc):
    ...
    metric_report(
        ...
        str(acc[0][0]),             # Demo Account (email)
        str(acc[0][3]),             # Country
        str(acc[0][4]),             # Language
        tree,                       # Domain / Module(s) run
        str(start_date),            # Start date
        str(end_date),              # End date
        str(no_of_links),           # no of links
        str(total_time/60)          # Total Runtime (in mins)
    )
