...
class PRP():
    base_url="https://partner.hpe.com/group/competitor"
    delayed_loading_links =work.doc_reader("competitor_delayed_loading.docx")
    delayed_loading_links=[s.strip() for s in delayed_loading_links if s!='']
    breadcrumblinks=work.doc_reader("competitor_breadcrumb_links.docx")
    breadcrumblinks=[s.strip() for s in breadcrumblinks if s!='']
    # print('BREADCRUMBS',breadcrumblinks)
    absurd_links = work.doc_reader("competitor_absurd_links.docx")
    absurd_links=[s.strip() for s in absurd_links if s!='']
    # print(absurd_links)
    breadcrumb_prefix=work.doc_reader("competitor_breadcrumb_prefix.docx")
    ...
    def __init__(self, username: str,password: str,region:str,country,language,acc_type):
        ...
        self.page_tree_path='CompetitorPageTrees\\PageTree{r}_{c}_{l}_{a}.txt'.format(r=self.region,c=self.country,l=self.language,a=self.account_type)
        ...
        self.doc_link_path ='Competitor DocumentLinks\\Doclinks{r}_{c}_{l}_{a}.txt'.format(r=self.region,c=self.country,l=self.language,a=self.account_type)
        self.reverse_dict_path='Competitor Reverse Dicts\\RevDict{r}_{c}_{l}_{a}.txt'.format(r=self.region,c=self.country,l=self.language,a=self.account_type)
        self.external_urls_path='Competitor External Urls\\External{r}_{c}_{l}_{a}.txt'.format(r=self.region,c=self.country,l=self.language,a=self.account_type)
        self.cp_links = None
    ...
    def scrape(self,queue,internal,external,allurls,doclinks,tree_dict):
        ...
        while queue:
            ...
            if link.startswith('http'):
                allurls.add(link)
                if (link.startswith("https://partner.hpe.com/group/competitor")):
                    ...
                    try:
                        ...
                        for ele in ahref:
                            ...
                            if url is not None and url.strip()!='' and url not in allurls and 'login' not in url and 'logout' not in url and '#' not in url and '?p_p_id=com' not in url :
                                if not url.startswith('http') and not url.startswith('https://partner.hpe.com') and ('article-display-page?' in url or 'group/competitor' in url):
                                    if url.startswith('/group/competitor'):
                                        url='https://partner.hpe.com'+url
                                        print(url)
                                    else:
                                        url = 'https://partner.hpe.com/group/competitor'+url
                                        print(url)
    ...
    def scrapecall_writetrees(self):
        ...
        self.cp_links = len(internal)+len(all_links)+len(external)+len(docs)
        with open(self.page_tree_path, 'w') as filehandle:
                for item in internal:
                    if item.startswith("https://partner.hpe.com/group/competitor"):
                        filehandle.write('%s\n' % item)
...
if __name__=='__main__':
    credentials = [['demo_competitor@pproap.com', 'Login2PRP!', 'NA', 'USA', 'English', 'CTD'],
                   ['demo_mapcompetitor_solp@yopmail.com', 'Login2PRP!', 'EMEA', 'Lithuania', 'English', 'T2']]
                  
                  
                #    ['demo_japanese_jp_competitor@pproap.com','Login2PRP!','APJ','Taiwan','Taiwan','CTD']]
