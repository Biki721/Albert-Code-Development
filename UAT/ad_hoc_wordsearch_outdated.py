...
def grammatical_errors(extracted_text,tbi,articletitles,language): #    errors = grammatical_errors(content,find_all_example,art_titles_tocheck,lang)
    ...
    if language =='English' or language == 'Singaporean':
        tbc = 'en'
        col_name = 'en-US'
    else:
        tbc = codes[language]
    ...
    aruba_names_df = pd.read_excel('Ad hoc Requests\Aruba Series names - Adhoc request.xlsx')
    aruba_names = aruba_names_df['Active Series name'].to_list()
    ...
    try:
        gramm_res = spell_gramm(gramm_checks, check_list, aruba_names)
        #print(gramm_res)
...
def spell_gramm(content, check_list, aruba_names):
