...
class PRP():
    ...
    def __init__(self, username: str, password: str, region: str, country, language, acc_type):
        ...
        self.redirect_log_path = f'Redirects\\Redirects{self.region}_{self.country}_{self.language}_{self.account_type}.txt'
    ...
    def handle_country_and_overlay(self, force_recheck=False):
        """Handle notification overlays and country selection."""
        page = self.session_page
        if page is None:
            logging.warning("⚠️ Session page not initialized")
            return ""
        ...
        try:
            ...
            if overlay and overlay.is_visible():
                if force_recheck:
                    logging.info("🔔 Closing notification overlay (re-check)")
                else:
                    logging.info("🔔 Closing notification overlay")
                try:
                    page.click("#closemsg", timeout=3000)
                except Exception:
            ...
            if not force_recheck:
                logging.warning(f"⚠️ Overlay handling error: {e}")
        ...
        try:
            ...
            if eyeball and eyeball.is_visible():
                eyeball.click()
        ...
        try:
            ...
            if country_element and country_element.is_visible():
                current_country = country_element.inner_text().strip()
                if force_recheck:
                    logging.info(f"🔄 Country re-check: {current_country}")
                else:
                    logging.info(f"🌍 Current Country: {current_country}")
        except Exception:
            if not force_recheck:
                logging.debug("Could not detect current country")
        ...
        try:
            if current_country and current_country.lower() == self.country.lower():
                if force_recheck:
                    logging.info(f"✅ Country still correctly set to '{current_country}'")
                else:
                    logging.info(f"✅ Country already set to '{current_country}'")
                ...
                if force_recheck:
                    logging.warning(f"⚠️ COUNTRY CHANGED! Resetting from '{current_country}' to '{self.country}'")
                ...
                if best_score >= 0.30 and best_option:
                    ...
                    logging.info(f"🌐 Country matched → '{best_name}' (score={best_score:.2f})")

                    try:
                        br_container = page.wait_for_selector("#MHMGBRLIst > li > div > div", timeout=15000)
                        if br_container and br_container.is_visible():
                            br_container.click()
                            page.wait_for_timeout(1500)  # Wait for page reload after country change
                    except Exception:
                        pass
                else:
                    logging.info(f"⚠️ No strong country match; best score={best_score:.2f}")
        except Exception as e:
            if not force_recheck:
                logging.warning(f"⚠️ Country switching error: {e}")

        # If country was changed, wait for page to stabilize
        if country_changed:
            try:
                page.wait_for_load_state("networkidle", timeout=10000)
                logging.info("✅ Page stabilized after country change")
            except Exception:
