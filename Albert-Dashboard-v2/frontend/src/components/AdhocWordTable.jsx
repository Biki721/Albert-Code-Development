...
import { adhocWordsAPI } from '../services/api';
import toast from 'react-hot-toast';

export default function AdhocWordTable() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await adhocWordsAPI.getTable();
        setTable(data);
      } catch (error) {
        toast.error('Failed to load adhoc word table');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      await adhocWordsAPI.saveTable(table);
      toast.success('Adhoc word table saved');
    } catch (error) {
      toast.error('Failed to save adhoc word table');
    } finally {
      setIsSaving(false);
    }
  };
